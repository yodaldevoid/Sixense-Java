...
public class Sixense {
    ...
    /**
     * Initializes the Sixense library. It must be called at least one time per application. Subsequent calls will have no effect.
     * Once initialized, the other Sixense function calls will work as described until <code>Sixense.exit()</code> is called.
     *
    ...
    /**
     * Shuts down the Sixense library.
     * After this method call, all Sixense API calls will return failure until <code>Sixense.init()</code> is called again.
     *
     * @return  true on success, false on failure
     */
    ...
    /**
     * Gets the maximum number of base units supported by the Sixense control system.
     *
     ...
     *
     * @param base_num  an integer from <code>0</code> to <code>Sixense.getMaxBases()-1</code>
     * @return          true on success, false on failure
     */
     ...
     *
     * @param controller_id an integer from 0 to <code>Sixense.getMaxControllers()</code>-1
     ...
     *
     * @param all_data  an initialized ControllerData[4] that the controller data will be put in
     * @return          true on as long as the system has been initialized, otherwise false
     */
    public static native boolean getAllNewestData(ControllerData[] all_data);

    /**
     * Gets state of all of the controllers, selecting how far back into a history of the last 10 updates.
     *
    ...
    /**
     * Gets the most recent state of one of the controllers.
     * If the desired controller is not connected, an empty data packet is returned.
     * Empty data packets are initialized to a zero position and the identity rotation matrix.
     *
     * @param controller_id an integer from 0 to <code>Sixense.getMaxControllers()</code>-1
     * @param data          a ControllerData that the controller data will be put in
     * @return              true on as long as the system has been initialized, false if the controller is not connected
     */
    public static native boolean getNewestData(int controller_id, ControllerData data);

    /**
     * Gets state of one of the controllers, selecting how far back into a history of the last 10 updates.
     * If the desired controller is not connected, an empty data packet is returned.
     * Empty data packets are initialized to a zero position and the identity rotation matrix.
     *
     * @param controller_id an integer from 0 to <code>Sixense.getMaxControllers()</code>-1
    ...
    /**
     * Sets the parameters that control the position and orientation filtering level.
     *
     * @param params
     * @return              true on as long as the system has been initialized, otherwise false
     */
    ...
    /**
     * Enables a period of tactile feedback using the vibration motor. Note the Razer Hydra does not support vibration.
     *
     * @param controller_id an integer from 0 to <code>Sixense.getMaxControllers()</code>-1
     * @param duration      the duration of the vibration, in 100 millisecond units
     * @param pattern_id    this argument is ignored and the vibration motor is engaged for the full duration
     * @return              true on as long as the system has been initialized, otherwise false
     */
     ...
     * This call is deprecieated, as hemisphere tracking is automatically enabled when the controllers are in the dock or by the <code>sixense.utils.ControllerManager</code>.
     * See the Sixense API Overivew for more information.
     *
     * @param controller_id this argument is ignored and the vibration motor is engaged for the full duration
     * @return              true on as long as the system has been initialized, otherwise false
    ...
    /**
     * Sets High Priority RF binding mode. This call is only used with the wireless Sixense devkits.
     *
     * @param on_or_off true to enable, false to disable
     * @return          true on as long as the system has been initialized, otherwise false
     */
    ...
    /**
     * Gets the current state of High Priority RF binding mode. This call is only used with the wireless Sixense devkits.
     *
     * @return  true if enabled, false if disabled
     */
    ...
    /**
     * Gets the color of the LED on the Sixense wireless devkits. Each value is between 0 and 255, 0 is off and 255 is fully on.
     *
     * @param colors
     * @return      true on as long as the system has been initialized, otherwise false
     */
